...
image::AddLoanSequenceDiagram.png[]
...
`addbike` checks if there's a `Bike` with the same name already in the LoanBook, and rejects the command if so.
...
The activity diagrams below demonstrates how this sub-feature could be implemented for the loaning out and returning of bicycles respectively:

image::BikeStatusAddLoanActivityDiagram.png[]

image::BikeStatusReturnLoanActivityDiagram.png[]
