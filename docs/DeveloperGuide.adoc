...
3. Tests were edited to include and check for a valid password before deletion.
...
* We reject the command if the given old password is not equal to password that is stored in the database. This is to ensure only the user who has the password would be allowed to change the password.
...
1. Storing the current password as a Password versus as a string.
