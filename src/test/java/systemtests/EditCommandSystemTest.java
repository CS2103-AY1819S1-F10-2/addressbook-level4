...
public class EditCommandSystemTest extends AddressBookSystemTest {
    ...
    public void edit() {
        ...
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_AMY + NRIC_DESC_BOB
                + PHONE_DESC_BOB + EMAIL_DESC_BOB + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        editedLoan = new LoanBuilder(BOB).withName(VALID_NAME_AMY).build();
        assertCommandSuccess(command, index, editedLoan);

        /* Case: edit a loan with new values same as another loan's values but with different NRIC, phone and email
         * -> edited
         */
        index = INDEX_SECOND_LOAN;
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + NRIC_DESC_AMY
                + PHONE_DESC_AMY + EMAIL_DESC_AMY + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        editedLoan = new LoanBuilder(BOB)
                .withNric(VALID_NRIC_AMY).withPhone(VALID_PHONE_AMY).withEmail(VALID_EMAIL_AMY).build();
        assertCommandSuccess(command, index, editedLoan);
        ...
        /* Case: edit a loan with new values same as another loan's values but with different NRIC -> rejected */
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + NRIC_DESC_AMY
                + PHONE_DESC_BOB + EMAIL_DESC_BOB + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        assertCommandFailure(command, EditCommand.MESSAGE_DUPLICATE_LOAN);
